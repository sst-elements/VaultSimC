...
class VaultSimC : public Component {
  ...
  public: // functions
    SST_ELI_REGISTER_COMPONENT(VaultSimC, "VaultSimC", "VaultSimC", SST_ELI_ELEMENT_VERSION(1, 0, 0),
                               "A memory vault in a stacked memory", COMPONENT_CATEGORY_MEMORY)

    SST_ELI_DOCUMENT_PARAMS({"clock", "Vault Clock Rate.", "1.0 Ghz"},
                            {"numVaults2",
                             "Number of bits to determine vault address (i.e. log_2(number of vaults per cube))"},
    ...
    SST_ELI_DOCUMENT_STATISTICS({"Mem_Outstanding", "Number of memory requests outstanding each cycle", "reqs/cycle",
                                 1})
