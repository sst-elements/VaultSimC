...
public: // functions
  SST_ELI_REGISTER_COMPONENT(VaultSimC, "VaultSimC", "VaultSimC",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"numVaults2", "Number of bits to determine vault address (i.e. "
                     "log_2(number of vaults per cube))"},
  ...
  SST_ELI_DOCUMENT_PORTS({"bus", "Link to the logic layer", {"MemEvent", ""}}, )

  SST_ELI_DOCUMENT_STATISTICS(
      {"Mem_Outstanding", "Number of memory requests outstanding each cycle",
       "reqs/cycle", 1})
...
private: // functions
  VaultSimC(const VaultSimC &c) = delete;
  ...
  t2MEMap_t transactionToMemEventMap; // maps original MemEvent to a Vault
                                      // transaction ID
...
#if HAVE_LIBPHX == 1
inline PHXSim::TransactionType
VaultSimC::convertType(SST::MemHierarchy::Command type) {
  ...
  switch( type )
    ...
    default:
      _abort(VaultSimC,"Tried to convert unknown memEvent request type (%d) to
  PHXSim transaction type \n", type);
  }
