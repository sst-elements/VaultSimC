...
VaultSimC::VaultSimC(ComponentId_t id, Params &params)
  ...
  if (-1 == nv2) {
    dbg.fatal(CALL_INFO, -1,
              "numVaults2 (number of bits to determine vault "
  ...
  PHXSim::VaultCompleteCB *readDataCB =
      new PHXSim::Callback<VaultSimC, void, BusPacket, unsigned>(
          this, &VaultSimC::readData);
  PHXSim::VaultCompleteCB *writeDataCB =
      new PHXSim::Callback<VaultSimC, void, BusPacket, unsigned>(
          this, &VaultSimC::writeData);
...
void VaultSimC::readData(BusPacket bp, unsigned clockcycle) {
  ...
  // printf("Burst length is %d. is that 64?: %s %d\n",bp.burstLength, __FILE__,
  // __LINE__); assert(bp.burstLength == parentEvent->getSize());

  // copy data from backing store to event
  // event->setSize(bp.burstLength);
  for (size_t i = 0; i < event->getSize(); i++) {
    // event->getPayload()[i] = memBuffer[getInternalAddress(bp.physicalAddress
    // + i)];
  }
...
void VaultSimC::writeData(BusPacket bp, unsigned clockcycle) {
  ...
  for (size_t i = 0; i < bp.burstLength; i++) {
    // memBuffer[getInternalAddress(bp.physicalAddress + i)] =
    // parentEvent->getPayload()[i];
  }
...
bool VaultSimC::clock_phx(Cycle_t current) {
  ...
  while ((e = m_memChan->recv())) {
    ...
    // add to the Q
    m_transQ.push_back(
        Transaction(transType, 64, event->getAddr(), thisTransactionID));
  }
...
bool VaultSimC::clock(Cycle_t /*current*/) {
  ...
  while (nullptr != (e = delayLine->recv())) {
    ...
    if (nullptr == event) {
      ...
      auto *respEvent = new MemRespEvent(event->getReqId(), event->getAddr(),
                                         event->getFlags());
