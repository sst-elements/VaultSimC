...
VaultSimC::VaultSimC(ComponentId_t id, Params &params) : Component(id), numOutstanding(0) {
    ...
    PHXSim::VaultCompleteCB *readDataCB =
        new PHXSim::Callback<VaultSimC, void, BusPacket, unsigned>(this, &VaultSimC::readData);
    PHXSim::VaultCompleteCB *writeDataCB =
        new PHXSim::Callback<VaultSimC, void, BusPacket, unsigned>(this, &VaultSimC::writeData);
...
bool VaultSimC::clock_phx(Cycle_t current) {
    ...
    while ((e = m_memChan->recv())) {
        ...
        dbg.output(CALL_INFO, " Vault %d got a req for %p (%lld %d)\n", vaultID, (void *)event->getAddr(),
                   event->getID().first, event->getID().second);
...
bool VaultSimC::clock(Cycle_t /*current*/) {
    ...
    while (nullptr != (e = delayLine->recv())) {
        ...
        if (nullptr == event) {
            ...
            auto *respEvent = new MemRespEvent(event->getReqId(), event->getAddr(), event->getFlags());
