...
logicLayer::logicLayer(ComponentId_t id, Params &params)
  ...
  dbg.init("@R:LogicLayer::@p():@l " + getName() + ": ", 0, 0,
           (Output::output_location_t)params.find("debug", 0));
  ...
  if (-1 == bwlimit) {
    dbg.fatal(CALL_INFO, -1, " no <bwlimit> tag defined for logiclayer\n");
  }

  int mask = params.find("LL_MASK", -1);
  if (-1 == mask) {
    dbg.fatal(CALL_INFO, -1, " no <LL_MASK> tag defined for logiclayer\n");
  }
  LL_MASK = mask;
  ...
  registerClock(frequency,
                new Clock::Handler<logicLayer>(this, &logicLayer::clock));
...
void logicLayer::init(unsigned int phase) {
  ...
  if (!phase) {
    toCPU->sendInitData(
        new SST::Interfaces::StringEvent("SST::MemHierarchy::MemEvent"));
  }

  // rec data events from the direction of the cpu
  SST::Event *ev = nullptr;
  while ((ev = toCPU->recvInitData()) != nullptr) {
    auto *me = dynamic_cast<MemEvent *>(ev);
    if (me) {
      /* Push data to memory */
      if (me->isWriteback() || me->getCmd() == Command::GetX) {
        ...
        if (me->getSize() > chunkSize) {
          ...
          for (int i = 0; i < numNewEv; ++i) {
            // make new event
            auto *newEv =
                new MemEvent(this, addr, me->getBaseAddr(), me->getCmd());
          ...
          if (isOurs(me->getAddr())) {
            // send to the vault
            unsigned int vaultID =
                (me->getAddr() >> VAULT_SHIFT) % m_memChans.size();
...
bool logicLayer::clock(Cycle_t current) {
  ...
  while ((tc[0] < bwlimit) && (e = toCPU->recv())) {
    ...
    if (isOurs(event->getAddr())) {
      // it is ours!
      unsigned int vaultID =
          (event->getAddr() >> VAULT_SHIFT) % m_memChans.size();
  ...
  for (auto m_memChan : m_memChans) {
    while ((e = m_memChan->recv())) {
      auto *event = dynamic_cast<MemRespEvent *>(e);
      if (event == nullptr) {
        dbg.fatal(CALL_INFO, -1, "logic layer got bad event from vaults\n");
      }
      //      dbg.output(CALL_INFO, "ll%d got an event %p from vault @ %lld,
      //      sends "
      dbg.output(CALL_INFO,
                 "ll%d got an event %p from vault @ %" PRIu64 ", sends "
                 "towards cpu\n",
                 llID, event, current);
  ...
  if (tm[0] > bwlimit || tm[1] > bwlimit || tc[0] > bwlimit ||
      tc[1] > bwlimit) {
