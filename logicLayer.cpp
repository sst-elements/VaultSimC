...
logicLayer::logicLayer(ComponentId_t id, Params &params) : Component(id), memOps(0) {
            ...
            if (me->isWriteback() || me->getCmd() == Command::GetX) {
                ...
                if (me->getSize() > chunkSize) {
                    ...
                    for (int i = 0; i < numNewEv; ++i) {
                        // make new event
                        auto *newEv = new MemEvent(this, addr, me->getBaseAddr(), me->getCmd());
                        ...
                        if (isOurs(newEv->getAddr())) {
                            // send to the vault
                            unsigned int vaultID = (newEv->getAddr() >> VAULT_SHIFT) % m_memChans.size();
